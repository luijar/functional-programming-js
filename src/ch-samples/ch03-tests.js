...
QUnit.test("CH03 - Recursion trees", function (assert) {
    ...
    var church = node(new Person('111-11-1231', 'Alonzo', 'Church'));

    var rosser = node(new Person('1111-11-1232', 'Barkley', 'Rosser'));
    var mendelson = node(new Person('111-11-1233', 'Elliot', 'Mendelson'));
    var sacks = node(new Person('111-11-1234', 'Gerald', 'Sacks'));

    var kleene = node(new Person('111-11-1235', 'Stephen', 'Kleene'));
    var nelson = node(new Person('111-11-1236', 'Nels', 'Nelson'));
    var constable = node(new Person('111-11-1237', 'Robert', 'Constable'));
    var clarke = node(new Person('111-11-1238', 'Edmund', 'Clarke'));
    var harper = node(new Person('111-11-1239', 'Robert', 'Harper'));

    church.append(rosser, kleene);
    rosser.append(mendelson, sacks);
    kleene.append(nelson, constable);
    constable.append(clarke, harper);

    var binaryTree = tree(church);

    var found = true;
    binaryTree.searchRoot(function (person) {
        console.log("Visiting : " + person.getFullName());
        if (person.getSsn() === '111-11-1236') {
            console.log('Found: ' + person.getFullName() + '!');
            assert.ok(found);
        }
    });
});
